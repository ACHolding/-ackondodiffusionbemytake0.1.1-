#!/usr/bin/env python3
# ackondosdiffusionbeemytake0.1.py
# AC Kondos Diffusion Bee 0.1 My Take — DiffusionBee-style desktop app (single file).
...
APP_TITLE = "AC Kondos Diffusion Bee 0.1 My Take"
APP_NAME = "AC Kondos Diffusion Bee 0.1 My Take"
APP_UA = "ACKondosDiffusionBee/0.1"
TITLEBAR_H = 55
SIDEBAR_W = 200
...
BTN_BG = LIGHT_THEME["BTN_BG"]
BTN_HOVER = LIGHT_THEME["BTN_HOVER"]
...
def apply_theme(dark: bool = False) -> None:
    ...
    global BLUE, BLUE_HOVER, BTN_BG, BTN_HOVER, ICON_BLUE, SIDEBAR, SIDEBAR_SEL, INPUT_BG, CONTENT
...
HOME = Path.home() / ".ackondos-diffusion-bee"
_LEGACY_HOME = Path.home() / ".openphoto"
LLM_DIR = HOME / "llms"
...
OUTPUT_DIR = os_pictures_dir() / "AC Kondos Diffusion Bee"
_LEGACY_OUTPUT_DIR = HOME / "outputs"
_LEGACY_PICTURES_OUTPUT = os_pictures_dir() / "OpenPhoto"
_LEGACY_OPENPHOTO_OUTPUTS = _LEGACY_HOME / "outputs"
...
def load_preview_photo(path: str, max_w: int = 420, max_h: int = 520) -> Optional[tk.PhotoImage]:
    """Fast preview thumbnail — sips on macOS, subsample fallback elsewhere."""
    p = Path(path)
    if not p.is_file():
        return None
    if _sys.platform == "darwin":
...
# Map AC Kondos Diffusion Bee FLUX labels → Pollinations model ids
FREE_FLUX_MODEL_MAP = {
...
# Pre-baked Hugging Face LLMs for FLUX prompt vibes (download on demand into ~/.ackondos-diffusion-bee/llms)
...
# DiffusionBee Homepage.vue categories + card copy (misc = AC Kondos extras)
HOME_SECTIONS = [
    (
        "All AI Tools",
        [
...
def _http_json(
    ...
    data = None
    hdrs = {"Accept": "application/json", "User-Agent": APP_UA}
    if headers:
        hdrs.update(headers)
    if body is not None:
...
class HuggingFaceLLMStore:
    """Pre-baked + vibe-added HF GGUF catalog; downloads into ~/.ackondos-diffusion-bee/llms."""
...
class AppConfig:
    ...
    def load(cls) -> "AppConfig":
        ...
        # One-time migrate OpenPhoto config/history/LLMs into the rebranded home
        try:
            if _LEGACY_HOME.exists():
                for name in ("config.json", "history.json", "hf_vibes.json", "hf_installed.json"):
                    src = _LEGACY_HOME / name
                    dst = HOME / name
                    if src.exists() and not dst.exists():
                        shutil.copy2(src, dst)
                legacy_llms = _LEGACY_HOME / "llms"
                if legacy_llms.is_dir() and not LLM_DIR.exists():
                    shutil.copytree(legacy_llms, LLM_DIR)
        except Exception:
            pass
        cfg = cls()
        ...
        legacy_outs = {
            _LEGACY_OUTPUT_DIR,
            HOME / "outputs",
            _LEGACY_OPENPHOTO_OUTPUTS,
            _LEGACY_PICTURES_OUTPUT,
            _LEGACY_HOME / "outputs",
        }
        if not cfg.output_dir or out in legacy_outs:
...
class FluxBackend:
    ...
    def _generate_free(
        ...
        req = urllib.request.Request(
            url,
            headers={"User-Agent": APP_UA, "Accept": "image/png"},
        )
        try:
...
class LLMBackend:
    ...
    def _pollinations_chat(self, model: str, messages: list[dict]) -> str:
        ...
        for url in urls:
            req = urllib.request.Request(
                url,
                headers={
                    "User-Agent": f"Mozilla/5.0 {APP_UA}",
                    "Accept": "text/plain",
    ...
    def _openai_chat(self, provider, model, messages, temperature) -> str:
        ...
        if provider == "openrouter":
            headers["HTTP-Referer"] = "https://localhost/ackondos-diffusion-bee"
...
class LLMDownloadWindow(tk.Toplevel):
    def __init__(self, app: "App"):
        ...
        tk.Label(
            head,
            text=f"Pre-baked Hugging Face GGUFs for FLUX vibes · download into ~/{HOME.name}/llms · vibe-add any repo",
        ...
        tk.Button(
            ...
            bg=BTN_BG, fg=BLUE, activebackground=BTN_HOVER, activeforeground=BLUE_HOVER,
        ...
        tk.Label(
            right,
            text=f"Weights stay local. Point Ollama / LM Studio / llama.cpp at ~/{HOME.name}/llms after download.",
...
class ToolWorkspace(tk.Toplevel):
    def __init__(self, app: "App", title: str, mode: str):
        ...
        self.model_hint.grid(row=2, column=0, columnspan=3, sticky="w", pady=(4, 0))
        ...
        self.gen_btn = tk.Button(
            ...
            command=self._generate, bg=BTN_BG, fg=BLUE,
            activebackground=BTN_HOVER, activeforeground=BLUE_HOVER,
    ...
    def _generate(self):
        ...
        if last_job:
            self._watching = last_job.id
            label = f"{batch} jobs queued" if batch > 1 else f"Job {last_job.id}: queued"
...
class App(tk.Tk):
    ...
    def _show_splash(self):
        ...
        tk.Label(
            wrap, text="AC Kondos Diffusion Bee", bg=CONTENT, fg=TEXT,
            font=(UI_FONT, 28, "bold"),
        ).pack()
        tk.Label(
            wrap, text="0.1 My Take", bg=CONTENT, fg=MUTED,
            font=(UI_FONT, 18),
        ).pack(pady=(4, 0))
        tk.Label(
            wrap, text="DiffusionBee-inspired · FLUX + Hugging Face LLMs",
            bg=CONTENT, fg=MUTED,
            font=(UI_FONT, 12),
        ).pack(pady=(18, 0))
    ...
    def _open_overflow_menu(self):
        ...
        m.add_separator()
        m.add_command(label=f"About {APP_NAME}", command=lambda: messagebox.showinfo(
            APP_NAME, f"{APP_NAME}\nDiffusionBee-inspired GUI\nFLUX + Hugging Face LLMs",
        ))
        try:
    ...
    def _build_activity_page(self, parent):
        ...
        tk.Label(header, text="Live job queue from the AC Kondos Diffusion Bee engine.",
    ...
    def _build_settings_page(self, parent):
        ...
        tk.Label(
            header, text=f"API keys & defaults. Saved to ~/{HOME.name}/config.json",
    ...
    def _make_card(self, grid, row, col, title, desc, kind, mode):
        ...
        btn = tk.Label(
            bottom, text="Open", bg=BTN_BG, fg=BLUE,
        ...
        btn.bind("<Enter>", lambda e, b=btn: b.configure(bg=BTN_HOVER, fg=BLUE_HOVER))
        btn.bind("<Leave>", lambda e, b=btn: b.configure(bg=BTN_BG, fg=BLUE))
